...
async function ensureSeed() {
  ...
  for (const employee of employees) {
    ...
    await pool.query(
      ...
      [id, companyId, employeeNumber, idCard, name, contact, site, role, salary, basicSalary, 1500, 0, 1800, 450, "2025-06-15"],
    );
  }
...
async function authenticate(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    const result = await pool.query<UserRecord & { expires_at: Date }>(
    ...
    req.auth = { id: user.id, name: user.name, email: user.email, role: user.role };
    req.token = token;
    return true;
  } catch (error) {
...
async function companyPayload(companyId: string) {
  const result = await pool.query(
    `SELECT c.*, COUNT(e.id)::int AS employee_count,
      COALESCE(SUM(a.total_billing),0) AS total_billing,
      COALESCE(SUM(a.total_receiving),0) AS total_receiving,
      COALESCE(SUM(a.cash_received),0) AS cash_received,
      COALESCE(SUM(a.salary),0) AS salary,
      COALESCE(SUM(a.expense),0) AS expense,
      COALESCE(SUM(a.dress_stock),0) AS dress_stock
     FROM companies c
     LEFT JOIN employees e ON e.company_id = c.id AND e.deleted_at IS NULL
...
export async function handleManagement(req: ApiRequest, res: ApiResponse): Promise<boolean> {
  ...
  if (params) {
    ...
    if (!body) return true;
    const result = await pool.query("SELECT id, name, email, role, password_hash FROM users WHERE lower(email) = $1", [body.email.toLowerCase()]);
    const row = result.rows[0];
  ...
  if (params) {
    ...
    await pool.query(
      ...
      [id, req.params.companyId, employeeNumber, idCard, body.name, body.contact, body.salary, body.site, body.role, body.basicSalary, body.allowances, body.overtime, body.pf, body.esic, body.profilePictureUrl ?? null, body.dateOfJoining],
    );
